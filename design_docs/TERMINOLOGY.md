...
- **Eidetic** — private local memory crate (formerly *Mnem*). Persistence layer for graph snapshots, traversal logs, settings, browsing memory. Every mere is backed by Eidetic (lexicon §4.1; Mark, 2026-09-23: "Eidetic is that dependency"): it saves a mere's sessions and archives them as codicils. Distinct from a Moot's shared lanes. Name evokes eidetic memory ("remembered with high fidelity"). The substrate codicils are distilled from.
...
- **servitor** — the resident helper unit: an installed extension or local agent, living as a node bearing a nested graph, holding a personae identity and a capability grant, proposing changes through the participant gate (so it cannot exceed its grant, and every act is attributed and revertible). Chosen by Mark 2026-07-17 for the chaos-magic sense: created, named, task-scoped, dissolvable. Crate name reserved on crates.io the same day (0.0.1). A human peer is never a servitor; both are *participants*. *Animula* (Hadrian's "guest and companion of the body") is banked as the companion-flavored runner-up. See the [participant gate and packs plan](mere_docs/implementation_strategy/2026-07-17_participant_gate_packs_plan.md)
- **participant** — anything admitted to act through the gate: a personae identity holding a grant and the right to submit petitions. Human moot peers, servitors, and scenario runners are participants; the trusted UI uses the direct attributed commit path. Restored by Mark 2026-09-20, superseding the 2026-07-17 use of *denizen* for this role. Existing `Denizen*` APIs, `mere.denizen` delegation domain, `denizen.binding` facets, author prefixes, action IDs, and saved paths retain their compatibility spellings; they describe participants and do not give the simulation term authority over the platform.
- **denizen** — reassigned by Mark 2026-09-20 to Isometry's simulation family: an inhabitant remembered individually because of its history, relationships, or explicit designation. The canonical definition and tier consequences live in `isometry/mesocosm/design_docs/2026-09-18_wing_design_plan.md`, §3.2.1. The term and prospective crate name belong to that simulation concept; this ruling creates no crate, transfers no Servitor implementation, and makes no crates.io availability claim.
- **petition** — a participant's proposed change: a typed batch (graph edits lowering to captured deltas, app effects as Actions) validated against the grant and the journal revision before atomic, attributed apply. The journal records granted petitions. Ruled 2026-07-17 (replaces the working word *proposal*)
- **watch** — a participant's subscription: the scope of the graph whose committed changes wake its body, with the containment law watch ⊆ read ⊆ grant (you cannot be woken by what you cannot read). Watches are declared in the pack manifest and reviewed at install beside the rings; a chain of wakes is a *cascade*, bounded by a budget that is a setting. Ruled by Mark 2026-08-13. Code note: `Watch` will collide with `tokio::sync::watch` and filesystem watchers; prefer `GraphWatch` or similar at the type level. See the [graph behaviors plan](mere_docs/implementation_strategy/2026-08-13_graph_behaviors_plan.md)
...
- **mere** *(lowercase, count noun)* — a configurable spatial dataspace holding one data domain: the unit applications integrate. **Amended 2026-09-23 (Mark):** "each data domain deserves its own mere. that isn't the same as every app getting its own mere, like two browsers should share a mere", and any of an identity's applications can open any of its meres by default ("at least exceptions should have a really good reason"). Cleromancy's divination, journaling and RNG domain is one mere. Mark named its neighbours by application: Knot (notes, seeds, prompts), Redshank (notes), Isocosm (seeds). Whether applications that share a subject, such as notes, also share one mere is not ruled. A mere is its domain's repository (Mark, 2026-09-23). It lives as **sessions**, the durable graph sessions Mere multiplexes. Eidetic, which backs every mere, saves those sessions and archives them as codicils; a session can be opened from a codicil, and editing forks it. A remote user can access or clone a mere according to their identifier and credentials. Meres are to sync, branch and merge, resolving conflicts, going forward. The earlier per-application examples (Isometry's overmap, Woodshed's stage, Strophe's arrangement, Turnstone's canvas) are retired. A user has many meres, and all of an identity's meres together are its **reservoir**. Capital **Mere** is the platform, lowercase **a mere** is one dataspace, and the platform is deliberately named for its unit. The word is the lake sense already carried in Mere's own positioning above (a small lake, still-water surface). **Amended 2026-08-13 (with Mark):** two borrowed terms of art apply on different axes, and a mere is genuinely both. *Dataspace* is the integration axis ([Franklin, Halevy & Maier, SIGMOD Record 34:4, 2005](https://dl.acm.org/doi/10.1145/1107499.1107502)): interrelated heterogeneous sources queried and navigated without full upfront integration, with relationships added *pay-as-you-go* — which is precisely cross-application linking between applications that never agree on a schema. *Datalake* is the storage axis (Dixon, 2010): raw native retention, schema-on-read, one accretive pool — of which this stack's *schema at the codicil boundary* is the sharper statement. The lake term's usual "derivative copies beside someone else's system of record" reading is deployment practice accreted onto Dixon's actual contrast (natural versus cleansed-and-bottled), not part of the definition, so it does not contradict a mere holding source truth. Where that connotation would mislead, say *governed lake*: the engineered impoundment with a catalog and a drain valve, which retention epochs, provenance, and native drop are what actually supply. Until 2026-09-23 this entry glossed a mere with IBM's word for the governed lake, *reservoir*, as unminted shorthand. That word is now minted at IBM's own scale, for an identity's whole governed set of meres (see **reservoir**). Ruled with Mark 2026-07-26: the concept had no name and was informally covered by *orrery* while the reference host was the only application, which is why the word stopped stretching once four products each integrated one. Tier 1 is *your root mere*, replacing *orrery* at that tier. Amended 2026-07-30: the tiers are escalating socialization of the mere itself, so solo is just a mere (not a moot-of-one), a moot is when your mere is shared, and sharing never stops a mere being a mere
...
- **ambiance** — ambient data: what an engine generates about the rest of a domain, around whatever a person has in focus, while it stays out of focus. Mark, 2026-09-23: "Ambiance can be a term that defines data in the background, ambient data". Isocosm ruling 70, 2026-09-20: "the background of information generated by an engine and localized to a particular address/place/scene". Mark's example is Wikipedia's unclicked links. Two independent axes place anything a person meets. **Attention** runs background → foreground → focus; it is view curation, per instance, and never synced. **Keeping** runs ambient → short-term → long-term → codicil. *Ambient* material is generated, regenerated on demand, never stored and never in conflict. Above it are Alembic's three levels of memory: short-term (*Recent*), which is evicted by default and is where examined material lands; long-term (*Saved*), reached by an affirmative promotion such as tagging or bookmarking; and **Codicil**, which is immutable, content-addressed and shareable. **The ambient tier** is keeping's bottom level, where ambiance sits. A move between keeping levels may be explicit, permissioned, scripted or automatic. Each move is recorded in the structural event log with who or what made it: the person, or the rule, script or engine with its version. The ambient tier is not one of the t1–t4 socialization tiers, and ambiance is not the physics ambient pulse or living backdrop, which paint system state behind the graph. *Keeping* is not **Standing**. Ruled with Mark 2026-09-23; design record: [ambiance design](mere_docs/design/2026-09-23_ambiance_design.md)
- **reservoir** — an identity's whole governed set of meres, one mere per data domain. Minted 2026-09-23 (Mark) at the scale of IBM's coinage: "The data reservoir is a reference architecture that balances the desire for easy access to data with information governance and security", for a whole organization ([IBM Redbook SG24-8274](https://www.redbooks.ibm.com/abstracts/sg248274.html)). Before that date the word appeared here only as shorthand for a mere read as a governed lake. A data domain is not a reservoir; it is one mere within one. The word is unrelated to *reservoir sampling*, the random-sampling algorithm
...
| denizen *(platform admission sense)* | **participant** | Reassigned 2026-09-20 to the Isometry simulation family; reverses the 2026-07-17 platform naming ruling. Compatibility identifiers retain their old spelling. |
