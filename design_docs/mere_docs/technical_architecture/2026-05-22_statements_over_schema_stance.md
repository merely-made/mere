...
- **Content graph.** What documents say, cite, contradict, derive from, are about. RDF-native. The `Semantic` family is close to CiTO already (`Cites` → `cito:cites`, `Quotes` → `cito:includesQuotationFrom`, `Contradicts` → `cito:disputes`, `Supports` → `cito:supports`; the evidence sense, ruled 2026-09-15); `Provenance` and `Imported` are PROV-O (`prov:wasDerivedFrom`); `tags` and classifications are SKOS. A mature standard vocabulary is already waiting for this half.
