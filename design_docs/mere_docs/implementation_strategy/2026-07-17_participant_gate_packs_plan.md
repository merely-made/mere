...
**Terminology amendment, 2026-09-20:** Mark reassigned *denizen* to Isometry's
individually remembered simulation inhabitants and restored *participant* for
this plan's gate actors. Current prose uses participant; historical receipts,
Rust API names, signed domains, persisted facets, paths, and action IDs retain
their existing spellings. See [TERMINOLOGY.md](../../TERMINOLOGY.md).
Static verification: executable Rust/WIT tokens and compatibility strings in
Mere are unchanged; only comments and the Servitor package description changed.
The active-doc audit reports exactly the same findings when checked against
the pre-edit tracked document text. Cargo checks remain held at the user's
request; this terminology pass adds no runtime receipt.
...
- **nested graph**: a graph, a set of relations, contained *within* a node. This is the only containment sense; when containment is meant, say nested graph and avoid the bare word "subgraph" (which, since the 2026-09-05 retirement of *graphlet*, names the selection sense).
- **subgraph** (ruled as *graphlet* on 2026-07-17; the word was retired for *subgraph* on 2026-09-05, see TERMINOLOGY.md): the existing forme mechanism, an ego/component *scope over real kernel nodes* with a Frontier (see the [relational browse graphlet plan](../../archive_docs/2026-08-06_completed_plans/2026-06-23_relational_browse_graphlet_plan.md)). A subgraph is peer-scoping, never containment, and is not the participant unit.
...
- **participant** (restored 2026-09-20, Mark): the umbrella gate-actor word: a personae identity holding a grant and the right to submit petitions. Human moot peers, servitors, and scenario runners are participants; the trusted UI uses the direct attributed commit path. The 2026-07-17 choice of *denizen* for this role is superseded; the founding filename already uses the current term.
- **petition** (DECIDED 2026-07-17, Mark): a participant's proposed change, replacing working *proposal*. The journal records granted petitions.
...
The founding rulings and the 2026-09-20 participant amendment are reflected in [TERMINOLOGY.md](../../TERMINOLOGY.md).
...
- **Attribution**: every committed batch records the participant's identity in the journal envelope; stemma carries provenance; scholia carries the user's own annotations on what a participant did. Honesty bound (2026-07-17): today's `GraphEdit` retains no inverses (a `RemoveNode` keeps neither the payload nor its edges), so the promise at B1 is **auditability plus compensating actions**, never universal undo; universal undo waits for a journal that records pre-state or reversible operations. Prefix replay reconstructs any historical state, but selective inversion of one batch with later edits retained is not available today.
...
- **Participant node anatomy**: the node body (`ContentBearing`) is the signed manifest blob. The nested graph holds: **grant projections** (readable statements projected by the gate, never the authority source: authority materializes from signed personae/moot/kith evidence per the kith capability sharing plan, and the graph is the explanation and local index. A self-editable authority root would let a servitor grant itself power, so the gate refuses petitions that touch grant projections), the storage root (a muniment namespace keyed by participant id), one journal-cursor node per event subscription (crash-restart resumes from the cursor), and registered commands/panes as nodes.
- **The payoff**: the palette is a query over participant nested graphs; "what can act on my data, what did it do, why do I trust it" is a browsing question; a swatch over the participant's nested graph *is* its inspection UI. The orrery examines itself.
- **Uninstall means archive/disable, never deletion** (2026-07-17): B0's archive move is the mechanism, and the participant's storage namespace and referenced blobs get an explicit retention rule under the one deletion/retention law (see the deletion, retention and native drop plan) rather than an implied purge.
...
- **It cannot hurt you**: the gate's guarantee, identical to every other participant: it is physically unable to exceed its grant, every act is attributed in the journal, and every act is auditable with compensating actions (universal undo arrives only with a pre-state-recording journal; see §1's honesty bound).
...
- **B0.5, the live gate seam** (chartulary + first consumer): the review's keystone insertion, because the gate is a design until the journal has a transactional, attributed batch API. Contents: (1) revision-checked batch commit on the journal (envelope: batch id + participant identity + expected seq; refusal carries the current revision for rebase; the log entry type gains an attributed envelope, and migration of existing bare-`GraphEdit` logs is decided here, see open question 7); (2) the post-commit effect queue discipline (idempotent app effects carrying the batch id, drained only after the graph batch lands); (3) authority materialization (signed personae/moot/kith evidence in, grant projections out into the participant's nested graph; petitions touching projections refused); (4) the B0 follow-ups: archived-pending-removal recovery and self-reference rejection at attach. Done when: a stale-revision batch is refused with the current revision; a committed batch reads back attributed to its participant; effects demonstrably drain only post-commit; a self-bearing attach is rejected; a simulated crash between archive and parent-save recovers idempotently.
...
5. RESOLVED in full (Mark, 2026-07-17): umbrella word = **participant**, proposal = **petition**, **pack** kept for shallow rungs with **mod** for rung 3+, swatch wording landed in TERMINOLOGY (with the correction that the gloss is a pane containing a swatch), and `GraphBearing` blessed as final. Candidate pools and availability are logged in the naming-round notes. RESOLVED for the helper unit: **servitor** (Mark, 2026-07-17; reserved on crates.io as 0.0.1). Runner-up banked: animula. Rest of the checked pool: free = paredros, ushabti/shabti, fylgja, wolpertinger; taken (all by agent/daemon frameworks) = famulus, duende, karakuri, holon, familiar.
...
8. **RESOLVED against the existing design (Mark pointed to prior docs, 2026-07-17): the participant grant is NOT a new type; it rides the already-designed three-layer capability stack.** My earlier "extract a kind-agnostic core from `DeviceGrantPayload` / adopt the Meadowcap shape into personae" was reinventing work already done. The canonical design is the **event-DAG substrate brief §8.8** (resolved 2026-06-03) plus the **graph-cluster-namespaces brief** (2026-05-10):
   ...
   - **`DeviceGrantPayload` (wallet_grant) is the orthogonal device tier** (which personas a device may act for), not the graph-area cap; it does not umbrella participants and should not be generalized to.
   - **Reality of the code**: the structural-cap layer is designed and the moot provider seam exists, but the mere-native meadowcap cap type is **not built** (no `willow-cluster-cap` probe despite the doc note; no `MerePath`/`McCapability` types; `mere-namespace` is prose). So B1's authority materialization uses the **provider seam** with a minimal `capability_covers` now (a participant holds a scoped structural cap over its granted graph area, checked as prefix/leaf-id coverage), and the full graph-cluster meadowcap layer lands when `mere-namespace` is built. **Participant residency's grant projection = a readable render of the participant's structural cap into its nested graph; the gate's authority = the provider's `capability_covers` for the participant subject.** No new grant model, no wallet_grant edit.
...
- **2026-07-24 (the doctrine's THIRD actor kind: a moot peer through the same
  ...
  participant. Receipts in gemot's `typed_authorization` (99 tests): in-scope
...
- **2026-07-23 (the turnstone world RULED: total surface, ring-gated envelope —
  ...
  **ring** — a capability-path family checked against the participant's grant at
...
- **2026-07-22 (fork CARRIES worlds)**: the follow-on to the containment
  ...
  the fork's participant rebuilds as a full resident with no legacy heal.
...
- **2026-07-22 (containment RULED structural — `Node.nested` + `GraphBearing`
  ...
  modeling objection (participant status = facet bundle = agency; containment =
  ...
  with `graph.json` carrying `nested` on the participant node and the binding
...
- **2026-07-22 (B1 COMPLETE — mere `e54ca8cf`, turnstone `8b3ad31`)**: the
  ...
  Gate's read-only grant projections, persisted per participant; **authority
...
- **2026-07-22 (B2 COMPLETE — turnstone `9727081`)**: the piccolo lane's
  `ScriptCapabilities` derive from the participant's structural caps (each class a
...
- **2026-07-22 (B3 bars VERIFIED on today's tree)**: document-host's suite
  ...
  a turnstone participant end-to-end; which Actions a component may emit is a
