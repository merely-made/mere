    ...
    V: RootView<State>,
{
    ...
    /// only as far as the node needs. Each is clamped to its range. Vertical
    /// only. A node that no longer exists or does not paint is a no-op;
    /// requests queued together resolve in order.
