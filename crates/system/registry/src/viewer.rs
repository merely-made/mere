...
//! path because it depends on `crate::viewer::app::GraphIntent`,
//! `crate::viewer::app::AppCommand`, `crate::viewer::prefs::FileAccessPolicy`, and
//! `crate::viewer::shell::desktop::workbench::*` — all binary-root concerns
//! that haven't been promoted.
//!
//! Dependencies in mere:
//! - `registry::layout::CapabilityDeclaration` (conformance vocabulary)
    ...
    pub fn select_for_uri(&self, uri: &str, mime_hint: Option<&str>) -> ViewerSelection {
        ...
        if mime_hint.is_none() {
            if let AddressKind::File = address_kind_from_url(uri) {
                ...
                // (was crate::viewer::shell::desktop::workbench::local_file_access::file_path_from_node_url).
