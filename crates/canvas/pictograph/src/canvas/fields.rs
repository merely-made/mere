...
//! Field-region interaction for [`Canvas`](crate::canvas::Canvas): which field the cursor
...
impl Canvas {
    ...
    pub(crate) fn rebuild_coupling_forces(&mut self) {
        let forces: Vec<CouplingForce> = self
            ...
            .filter_map(|c| {
                ...
                crate::canvas::seiche_bridge::coupling_force_from_graph(c, &self.graph)
            })
            .collect();
    ...
    pub fn delete_field(&mut self, id: FieldId) {
        ...
        if retired {
            ...
            self.physics.settle(crate::canvas::SETTLE_TICKS / 3);
        }
    }
