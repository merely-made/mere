...
| `distillery_chronicle_w1.json` | Distillery Chronicle snapshot, same-session rediscovery/resume by diff, and frozen table | byte-checked by `distillery_w1`; `distillery_chronicle_host` proves admitted serving; the external `distillery_chronicle_w1.scn` proves the readable three-card fixture over WebRTC, and `distillery_chronicle_live_w1.scn` proves the live Djinn-owned one-job route over the same door with zero overlap |
| `distillery_chronicle_w2.scn` | one authored Chronicle definition, two host-read datasets, shelfmark generation checks, and the `era-bands-off` variant | the 2026-09-08 headed Chrome run passed 18/18 steps; `loader.js` accepted `mere.chronicle-binding-receipt/1`, required two successful reads bound to the definition, matched each shelfmark authority and `expects_generation` to its source, matched the live Djinn generation to the mounted scene, and accepted only the one-option `era-bands-off` override. The full binding JSON, browser result, and 1282 by 722 capture are under `C:\Users\mark_\Code\testing\mere\scenarios\graphshell-web\distillery_chronicle_w2`. |
